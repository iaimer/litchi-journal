...
- `CHANGELOG.md` — 版本发布记录
- `SESSION_LOG.md` — 逐次开发会话记录

## 文档更新工作流

推送或发布前，按以下顺序更新，禁止跳过：

1. 版本递增时更新 `pubspec.yaml` 的 `version`。
2. 有架构/约束/决策变更时更新 `AGENTS.md` 对应章节。
3. 每次开发会话结束追加 `SESSION_LOG.md`（格式：`## YYYY-MM-DD 标题` + 讨论内容/决策&原因/改动文件清单/遇到的问题/最终结果）。
4. 有新版本时更新 `CHANGELOG.md`（按版本、按功能分组）。
5. 有功能/配置/结构变更时同步 `README.md`。
