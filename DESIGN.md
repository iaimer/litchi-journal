...
### Today Date And Coach

- 今天页日期使用“月日 + 星期”的两级标题，历史详情额外显示年份；日期层级应适配系统字体放大，不能依赖单行缩略。
- 人生教练使用中性开放式阅读区：与页面共享底色，不使用内层纸面填充、圆角或长左线；通过 Flora 小标题、正文层级和段落留白建立阅读结构，不使用完整彩色标题卡或常驻重阴影。
- 人生教练的五个内部模块在展示层使用 Flora 图标和纯文本标题；Markdown 中的旧 emoji 标题只作为兼容输入，不直接成为界面主视觉。
