...
当前工作区说明：`main` 已包含下一阶段「专注计时与时长统计」以及首页习惯紧凑布局修复，但尚未递增版本号或更新发布日志。发布版本仍按上面的 `1.6.6+25` 识别；本轮已构建 APK，不安装到真机。
...
- 专注计时（当前工作区未发布）：阅读/亲子阅读、学语言和自定义习惯可切换为时长模式；支持单会话暂停/恢复、后台时间戳恢复、手动追加/设为/清零，按开始日期写回分钟，并在统计页展示累计与平均时长；首页习惯任务恢复原始紧凑行距
...
短期优先验证画廊连续浏览的流畅度、图片失败重试、内存占用、深色模式，以及历史补录的数据完整性和多图上传体验。
