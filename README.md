...
当前工作区说明：本分支已实现下一阶段「专注计时与时长统计」，但尚未递增版本号、更新发布日志或推送远端。发布版本仍按上面的 `1.6.6+25` 识别；本轮只构建 APK，不安装到真机。
...
- 专注计时（当前工作区未发布）：阅读/亲子阅读、学语言和自定义习惯可切换为时长模式；支持单会话暂停/恢复、后台时间戳恢复、手动追加/设为/清零，按开始日期写回分钟，并在统计页展示累计与平均时长
