...
当前工作区说明：`1.7.3` 在 `1.7.2` 的习惯输入与标签管理基础上，完成 UI 去 AI 化阶段 1、阶段 2 和阶段 3 的展示层调整。本轮不递增版本号；Release APK 仍按正式发布清单单独构建与验收。
...
- 专注计时：阅读/亲子阅读、学语言和自定义习惯可切换为时长模式；支持单会话暂停/恢复、后台时间戳恢复、手动追加/设为/清零，按开始日期写回分钟，并在统计页展示累计与平均时长；首页习惯任务恢复原始紧凑行距
